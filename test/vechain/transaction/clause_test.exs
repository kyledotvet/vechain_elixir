defmodule Vechain.Transaction.ClauseTest do
  ...
  describe "ExRLP.Encode implementation" do
    test "encodes Clause struct correctly with no value" do
      clause = %Clause{
        to: <<0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB,
  ...
  describe "parse_clauses/2" do
    ...
    test "parses TxData structs in clauses" do
      ...
      assert clause.to == <<0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD>>
      assert clause.value == <<>>
    ...
    test "parses mixed clause types" do
      ...
      clause_struct = %Clause{to: <<0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56, 0x78>>, value: <<1>>, data: <<>>}
      ...
      assert %Clause{to: <<0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA, 0xAA>>, value: <<>>, data: <<0xAA>>} = c2
      assert %Clause{to: <<0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB, 0xBB>>, value: value, data: <<0xBB>>} = c3
      assert value == :binary.encode_unsigned(1000)
    end

    test "preserves other config fields" do
      config = %{chain_tag: <<0x4A>>, nonce: <<1, 2, 3>>}
      clause = %Clause{to: <<0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56, 0x78>>, value: <<1>>, data: <<>>}
      opts = [clauses: [clause]]
  ...
  describe "parse_clause/1" do
    test "returns Clause struct unchanged" do
      clause = %Clause{to: <<0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF, 0x12, 0x34, 0x56, 0x78>>, value: <<1>>, data: <<>>}

      result = Clause.parse_clause(clause)
    ...
    test "converts TxData to Clause for payable function" do
      ...
      result = Clause.parse_clause(tx_data)

      assert %Clause{} = result
      assert result.to == <<0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD, 0xEF, 0xAB, 0xCD>>
      assert result.value == <<>>
    ...
    test "converts TxData to Clause for non-payable function" do
      ...
      assert %Clause{} = result
      assert result.to == <<0x12, 0x34, 0x56, 0x78, 0x90, 0x12, 0x34, 0x56, 0x78, 0x90, 0x12, 0x34, 0x56, 0x78, 0x90, 0x12, 0x34, 0x56, 0x78, 0x90>>
      assert result.value == <<>>
    ...
    test "converts {TxData, value} tuple to Clause" do
      ...
      result = Clause.parse_clause({tx_data, 10_000})
    ...
    test "converts {TxData, value} tuple with zero value" do
      ...
      result = Clause.parse_clause({tx_data, 0})
